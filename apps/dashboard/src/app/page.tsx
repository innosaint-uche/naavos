...
const steps = [
  { num: "01", icon: Terminal, title: "Init", desc: "pnpm exec naavos init\nCreates ~/.naavos/avatar.json" },
  { num: "02", icon: Settings, title: "Configure", desc: "Edit avatar.json\nIdentity, rules, adapters, privacy" },
  { num: "03", icon: Link2, title: "Compile & Install", desc: "naavos compile --target hermes\nnaavos install --target hermes" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const t1 = gsap.fromTo(".hero-title", { y: 60, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, stagger: 0.08, ease: "power3.out" });
    const t2 = gsap.fromTo(".hero-badge", { scale: 0.9, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.6, delay: 0.15, ease: "back.out(1.7)" });
    const t3 = gsap.fromTo(".terminal-preview", { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, delay: 0.4, ease: "power2.out" });
    const t4 = gsap.fromTo(".feature-card", { y: 40, opacity: 0 }, {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold bg-gradient-to-r from-violet-400 to-purple-400 bg-clip-text text-transparent">NAAvOS</span>
            </div>
      ...
      <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="hero-title text-4xl sm:text-5xl lg:text-7xl font-bold tracking-tight mb-6">
            <span className="bg-gradient-to-r from-white via-zinc-200 to-zinc-400 bg-clip-text text-transparent">Give Every AI</span>
            <br />
            <span className="bg-gradient-to-r from-violet-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">Your Brain</span>
          </h1>
          ...
          <div className="terminal-preview max-w-3xl mx-auto rounded-2xl border border-white/10 bg-zinc-900/80 backdrop-blur-xl overflow-hidden shadow-2xl shadow-violet-500/10 text-left">
            ...
            <div className="p-4 sm:p-6 font-mono text-xs sm:text-sm">
              ...
              <p className="text-violet-400 mt-3 sm:mt-4">✓ Avatar Source Package created at ~/.naavos/avatar.json</p>
              <p className="text-zinc-500 mt-2">$ pnpm exec naavos compile --target hermes</p>
              <p className="text-cyan-400 mt-1">✓ Compiled → hermes (SOUL.md, SKILL.md, memories/)</p>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-12 sm:mb-16">Sound familiar?</h2>
              ...
              <ul className="space-y-3 sm:space-y-4">
                {[
                  ...
                  <li key={i} className="flex items-start gap-3 text-zinc-400 text-sm sm:text-base">
                    <span className="text-red-400 mt-1">→</span><span>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="space-y-3 sm:space-y-4">
                {[
                  ...
                  <li key={i} className="flex items-start gap-3 text-zinc-300 text-sm sm:text-base">
                    <span className="text-emerald-400 mt-1">→</span><span>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      {/* Features */}
      <section id="features" ref={featuresRef} className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {features.map((feature) => (
              <div key={feature.title} className="feature-card p-4 sm:p-6 rounded-2xl border border-white/10 bg-zinc-900/50 hover:border-violet-500/30 transition-all duration-300">
      ...
      <section id="how-it-works" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 sm:gap-8">
            {steps.map((step) => (
              <div key={step.num} className="p-6 sm:p-8 rounded-2xl border border-white/10 bg-zinc-900/50">
                ...
                <p className="text-xs sm:text-sm text-zinc-400 font-mono whitespace-pre-line">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
